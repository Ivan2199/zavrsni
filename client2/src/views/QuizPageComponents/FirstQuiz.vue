    ...
    <div class="BorderGlow">
      <article>
        <div class="quiz-wrap">
          ...
          <div
            class="quiz-main"
            v-for="question in quiz1.questions.slice(x, y)"
            :key="question.id"
            v-show="quizStart"
          >
            <div class="quiz-question">
              <h2>Pitanje {{ y }}/{{ quiz1.questions.length }}</h2>
              <h1>{{ question.text }}</h1>
              <img
...
export default {
  ...
  mounted: function () {
    axios
      ...
      .then((response) => (this.quiz1 = response.data))
  ...
  methods: {
    ...
    nextQuestion() {
      ...
      if (this.quiz1.questions.length - 1 == this.x) {
        this.quizEnd = true
