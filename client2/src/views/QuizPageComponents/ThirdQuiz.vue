    ...
    <div class="BorderGlow">
      <article>
        <div class="quiz-wrap">
          ...
          <div
            class="quiz-main"
            v-for="question in quiz3.questions.slice(x, y)"
            :key="question.id"
            v-show="quizStart"
          >
            <div class="quiz-question">
              <h2>Pitanje {{ y }}/{{ quiz3.questions.length }}</h2>
              <h1>{{ question.text }}</h1>
              <img
...
export default {
  ...
  mounted: function () {
    axios
      .get('http://localhost:8081/quiz/3')
      .then((response) => (this.quiz3 = response.data))
  ...
  methods: {
    ...
    nextQuestion() {
      ...
      if (this.quiz3.questions.length - 1 == this.x) {
        this.quizEnd = true
